...
$oids = snmpwalk_cache_multi_oid($device, 'juniSystemTempValue', array(), 'Juniper-System-MIB');

foreach ($oids as $index => $entry)
{
  if (is_numeric($entry['juniSystemTempValue']) && is_numeric($index) && $entry['juniSystemTempValue'] > 0)
  {
    $entPhysicalIndex = snmp_get($device, "juniSystemTempPhysicalIndex.$index", '-Oqv', 'Juniper-System-MIB');

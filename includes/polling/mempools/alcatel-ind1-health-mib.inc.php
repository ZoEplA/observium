...
// NOTE. Because Alcatel changed their MIBs content (same oid names have different indexes), here we only use numeric OIDs.

if (!$mempool['mempool_hc'])
{
  // Old AOS - don't strip mib_dirs() call!
  $mempool['total'] = snmp_get($device, '.1.3.6.1.4.1.6486.800.1.1.1.2.1.1.3.4.0', '-OvQ', 'ALCATEL-IND1-SYSTEM-MIB', mib_dirs('aos'));
  $mempool['perc']  = snmp_get($device, '.1.3.6.1.4.1.6486.800.1.2.1.16.1.1.1.10.0', '-OvQ', 'ALCATEL-IND1-HEALTH-MIB', mib_dirs('aos'));
} else {
  // New AOS - don't strip mib_dirs() call!
  ...
  $mempool['perc']  = snmp_get($device, '.1.3.6.1.4.1.6486.801.1.2.1.16.1.1.1.1.1.8.0', '-OvQ', 'ALCATEL-IND1-HEALTH-MIB', mib_dirs('aos7'));
}

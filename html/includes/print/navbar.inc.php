...
function navbar_entry($entry, $level = 1)
{
  ...
  else if (isset($entry['userlevel']) && isset($_SESSION['userlevel']) && $_SESSION['userlevel'] < $entry['userlevel'])
  {
     // skip not permitted menu items
     //continue;
  }
  else if (OBSERVIUM_EDITION == 'community' && isset($entry['community']) && $entry['community'] === FALSE)
  {
     // Skip not exist features on community
     //continue;
  }
  else if ($entry['locations']) // Workaround until the menu builder returns an array instead of echo()
  {

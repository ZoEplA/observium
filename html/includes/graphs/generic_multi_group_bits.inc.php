...
if($vars['perc'] || $vars['perc_agg']) { $scale_min = 0; }
...
foreach($data AS $id => $group)
{
  ...
  if(!isset($group['colours_in']))  { $group['colours_in']  = $config['group_colours'][$g_i]['in']; }
  if(!isset($group['colours_out'])) { $group['colours_out'] = $config['group_colours'][$g_i]['out']; }

  foreach ($group['rrds'] as $rrd)
  {

    if (!$config['graph_colours'][$group['colours_out']][$c_i] || !$config['graph_colours'][$group['colours_in']][$c_i]) { $c_i = 0; }
    ...
    $rrd_options .= " DEF:o_" . $ds_in ."=" . $rrd['file'].":".$rrd['ds_in'].":AVERAGE ";         // Load data
    $rrd_options .= " DEF:o_" . $ds_out ."=" . $rrd['file'].":".$rrd['ds_out'].":AVERAGE ";       // Load data

    $rrd_options .= " CDEF:" . $ds_in .  "=o_" . $ds_in . ",UN,0,o_" . $ds_in . ",IF";            // Strip out NaN values
    $rrd_options .= " CDEF:" . $ds_out . "=o_" . $ds_out . ",UN,0,o_" . $ds_out . ",IF,-1,*";     // Strip out NaN values

    $ds_io       = "io_".$i;

    $rrd_options .= " CDEF:" . $ds_io . "=o_" . $ds_in . ",UN,0,o_" . $ds_in . ",IF,o_" . $ds_out . ",UN,0,o_" . $ds_out . ",IF,+";
  ...
  $rrd_options .= " CDEF:" . $group_ds . "_in="  . implode(',', $group['ds_list_in']) . str_repeat(',+', count($group['ds_list_in']) - 1);
  $rrd_options .= " CDEF:" . $group_ds . "_out=" . implode(',', $group['ds_list_out']). str_repeat(',+', count($group['ds_list_out']) - 1);
  $rrd_options .= " CDEF:" . $group_ds . "_io="  . implode(',', $group['ds_list_io']) . str_repeat(',+', count($group['ds_list_io']) - 1);
...
$rrd_options .= " CDEF:agg_in="  . implode(',', $group_ds_in) . str_repeat(',+', count($group_ds_in) - 1);
$rrd_options .= " CDEF:agg_out=" . implode(',', $group_ds_out). str_repeat(',+', count($group_ds_out) - 1);
$rrd_options .= " CDEF:agg_io="  . implode(',', $group_ds_io) . str_repeat(',+', count($group_ds_io) - 1);
